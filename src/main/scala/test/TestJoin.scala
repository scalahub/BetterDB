...
object TestJoin extends App {
  ...
  val (uid, email, value, num) = (Col("uid", VARCHAR(255)), Col("email", VARCHAR(255)), Col("value", UScalaBIGINT(100)), Col("num", ULONG))
  implicit val config = MyDBConfig
  val dbm1 = Tab.withName("users1").withCols(uid, email, value, num).withPriKey(uid)
  val dbm2 = Tab.withName("users2").withCols(uid, value).withPriKey(uid)
  try {
